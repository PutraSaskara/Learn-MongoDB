...
exports.signup = (req, res, next) => {
    ...
    // generate random code for user password
    bcrypt.hash(password, 8, function(err,hash) {
        ...
        const newUser = new User({
            ...
            res.json({message: "User sukses register",userId:result._id})
...
exports.logout = (req, res, next) => {
    try {
        UserToken.findOneAndDelete({token: req.cookies['refreshToken']}).then(refToken => {
        if (!refToken)
            ...
            res.header('Clear-Site-Data', '"cookies", "storage"')
            .status(200).json({error:false, message: "Logged Out Sucessfully"})
        })
    } catch (err) {
        console.log(err)
...
const generateTokens = (user) => {
    try {
      ...
      const accesToken = jwt.sign(payload, process.env.ACCESS_TOKEN_PRIVATE_KEY, {
        expiresIn: "14m",
      });
      ...
      return newtoken
        .save()
        .then((res) => {
          console.log("login and new user token saved");
